...
function save(filePath) {
  ...
  import_fs5.default.writeFileSync(filePath, json);
  return filePath;
}
...
function load(filePath) {
  const json = import_fs6.default.readFileSync(filePath, "utf-8");
...
var init_Replay = __esm({
  "src/Replay/index.ts"() {
    ...
    Replay = class {
      ...
      export(format3) {
        return exportReplay.call(this, format3);
      }
    };
  }
});
...
var import_module, import_meta2, NodePTY, NodePTY_default;
  ...
  "src/PTY/NodePTY.ts"() {
    ...
    NodePTY = class {
      ...
      constructor(command2, args, options) {
        ...
        } else if (typeof import_meta2 !== "undefined" && import_meta2.url) {
          const requireFunc = (0, import_module.createRequire)(import_meta2.url);
          try {
        ...
        this.pty = nodePty.spawn(command2, args, {
...
// src/Battle/spawn.ts
async function spawn(command2, args = []) {
  this.log("info", `Spawning: ${command2} ${args.join(" ")}`);
  ...
  this.replay.record({
    ...
    data: { command: command2, args }
  });
  this.replay.data.metadata.command = command2;
  this.replay.data.metadata.args = args;
  this.pty = await createPTY(command2, args, {
    name: "xterm-256color",
...
function screenshot(name) {
  ...
  const filepath = import_path6.default.join(this.options.screenshotDir, filename);
  import_fs7.default.mkdirSync(this.options.screenshotDir, { recursive: true });
  const screen = parseTerminalOutput(this.output);
  import_fs7.default.writeFileSync(filepath, this.output);
  const cleanPath = filepath.replace(".txt", "-clean.txt");
  import_fs7.default.writeFileSync(cleanPath, screen.clean);
  const htmlPath = filepath.replace(".txt", ".html");
  import_fs7.default.writeFileSync(htmlPath, screen.html);
  this.replay.record({
    type: "screenshot",
    timestamp: 0,
...
`;
  if (!import_fs8.default.existsSync(this.options.logDir)) {
    import_fs8.default.mkdirSync(this.options.logDir, { recursive: true });
  }
  const logFile = import_path7.default.join(this.options.logDir, `battle-${this.startTime}.log`);
  import_fs8.default.appendFileSync(logFile, logEntry);
...
async function run(testFn) {
  ...
  try {
    ...
    replayPath = import_path8.default.join(this.options.logDir || "./logs", replayFilename);
...
var init_Battle = __esm({
  "src/Battle/index.ts"() {
    ...
    Battle = class {
      ...
      async spawn(command2, args) {
        return spawn.call(this, command2, args);
      }
      interact(handler) {
...
var parser = new YargsParser({
  ...
  require: (path6) => {
    if (typeof require !== "undefined") {
      ...
      return JSON.parse((0, import_fs2.readFileSync)(path6, "utf8"));
    } else {
...
var Silent = class {
   ...
   */
  exec(command2, options = {}) {
    try {
      const stdout = (0, import_child_process.execSync)(command2, {
        encoding: "utf8",
        ...options
      });
      this.log("info", `Executed: ${command2}`);
      ...
      this.log("error", `Failed: ${command2}`);
  ...
  isRunning(pattern) {
    try {
      const command2 = process.platform === "win32" ? `tasklist | findstr /i "${pattern}"` : `ps aux | grep -v grep | grep "${pattern}"`;
      const result = (0, import_child_process.execSync)(command2, {
        encoding: "utf8",
        stdio: "pipe"
      });
   ...
   */
  fileExists(filepath) {
    return import_fs9.default.existsSync(filepath);
  }
  /**
   * Read file content
   */
  readFile(filepath) {
    return import_fs9.default.readFileSync(filepath, "utf8");
  }
  /**
...
init_colors();
var argv = yargs_default(hideBin(process.argv)).command("test [file]", "Run tests from a file or directory", (yargs) => {
...
async function main() {
  const command2 = argv._[0];
  if (command2 === "test") {
    const testPath = argv.file || "./test";
    if (import_fs10.default.existsSync(testPath)) {
      const stats = import_fs10.default.statSync(testPath);
      if (stats.isDirectory()) {
        const files = import_fs10.default.readdirSync(testPath).filter((f) => f.endsWith(".test.ts") || f.endsWith(".test.js"));
        for (const file of files) {
          console.log(color.blue(`Running ${file}...`));
          const fullPath = import_path9.default.join(testPath, file);
          if (file.endsWith(".ts")) {
            try {
              const { spawn: spawn3 } = await import("child_process");
              const child = spawn3("tsx", [fullPath], { stdio: "inherit" });
              await new Promise((resolve5, reject) => {
                child.on("close", (code) => {
                  if (code === 0) resolve5(void 0);
                  else reject(new Error(`tsx exited with code ${code}`));
                });
                child.on("error", reject);
              });
            } catch {
              console.error(color.red("TypeScript files require tsx to be installed: npm install -g tsx"));
              process.exit(1);
            }
          } else {
        ...
        if (testPath.endsWith(".ts")) {
          try {
            const { spawn: spawn3 } = await import("child_process");
            const child = spawn3("tsx", [testPath], { stdio: "inherit" });
            await new Promise((resolve5, reject) => {
              child.on("close", (code) => {
                if (code === 0) resolve5(void 0);
                else reject(new Error(`tsx exited with code ${code}`));
          ...
          await import(import_path9.default.resolve(testPath));
        }
      }
    } else {
      console.error(color.red(`Test file not found: ${testPath}`));
      process.exit(1);
    }
    ...
    const result = await battle.run(async (b) => {
      ...
      if (argv.screenshot) {
        setTimeout(() => b.screenshot("cli-test"), 1e3);
      }
    });
    if (result.success) {
      console.log(color.green("Test passed"));
      if (result.replayPath) {
        console.log(color.blue(`Replay saved: ${result.replayPath}`));
        console.log(color.gray(`To replay: battle replay play ${result.replayPath}`));
      }
    } else {
      console.log(color.red("Test failed:"), result.error);
      if (result.replayPath) {
        console.log(color.blue(`Replay saved: ${result.replayPath}`));
        console.log(color.gray(`To replay: battle replay play ${result.replayPath}`));
      }
  ...
  } else if (command2 === "silent") {
    const cmd = argv.command;
    const silent = new Silent();
    const result = silent.exec(cmd);
    if (result.success) {
      console.log(color.green("Command succeeded"));
      console.log(result.stdout);
    } else {
      console.log(color.red("Command failed"));
      console.log(result.stderr);
      process.exit(1);
    }
  } else if (command2 === "replay") {
    const action = argv.action;
    const file = argv.file;
    if (action === "play") {
      if (!file) {
        console.error(color.red("Replay file path required"));
        process.exit(1);
      }
      if (!import_fs10.default.existsSync(file)) {
        console.error(color.red(`Replay file not found: ${file}`));
        process.exit(1);
      }
      ...
      const replay = new Replay();
      replay.load(file);
      const format3 = argv.format;
      const output = replay.export(format3);
      const outputFile = file.replace(".json", `.${format3}`);
      import_fs10.default.writeFileSync(outputFile, output);
      console.log(color.green(`Exported to: ${outputFile}`));
    }
  } else {
