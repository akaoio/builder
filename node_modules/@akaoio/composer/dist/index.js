...
async function data() {
  ...
  if (!await import_fs.promises.access(dataPath).then(() => true).catch(() => false)) {
...
async function loadDirectory(dirPath, data2, basePath) {
  const entries = await import_fs.promises.readdir(dirPath, { withFileTypes: true });
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const content = await import_fs.promises.readFile(fullPath, "utf-8");
      const ext = import_path.default.extname(entry.name).toLowerCase();
      const name = import_path.default.basename(entry.name, ext);
      let parsedData;
      switch (ext) {
        case ".json":
        ...
        case ".yaml":
        case ".yml":
          parsedData = import_js_yaml.default.load(content);
          break;
        case ".md":
        case ".markdown":
      ...
      parsedData._meta = {
        ...
        name,
        ext,
        relativePath: import_path.default.relative(basePath, fullPath)
      };
...
async function render2() {
  ...
  if (!await import_fs2.promises.access(templatesPath).then(() => true).catch(() => false)) {
    console.log(`Templates directory not found: ${templatesPath}`);
    return outputs;
  }
  const templateFiles = await getTemplateFiles(templatesPath);
  for (const templateFile of templateFiles) {
    const templateContent = await import_fs2.promises.readFile(templateFile, "utf-8");
    const template = new Template(templateContent, this.context);
    const rendered = template.render();
    const relativePath = import_path2.default.relative(templatesPath, templateFile);
    const outputName = import_path2.default.basename(relativePath, import_path2.default.extname(relativePath));
    outputs.set(outputName, rendered);
    if (this.options.outputPath) {
      const outputPath = import_path2.default.join(this.options.outputPath, `${outputName}.md`);
      await import_fs2.promises.mkdir(import_path2.default.dirname(outputPath), { recursive: true });
      await import_fs2.promises.writeFile(outputPath, rendered, "utf-8");
...
async function resolveImport(importConfig, baseDir) {
  ...
  const importPath = import_path3.default.isAbsolute(importConfig.source) ? importConfig.source : import_path3.default.resolve(resolveDir, importConfig.source);
...
function parseYamlImports(content) {
  try {
    ...
    const data2 = import_js_yaml2.default.load(content);
    const imports = [];
    findImports2(data2);
    return imports;
  } catch (error) {
...
function parseMarkdownImports(content) {
  ...
  if (frontmatterMatch) {
    try {
      const frontmatter = import_js_yaml2.default.load(frontmatterMatch[1]);
...
async function loadImportedFile(filePath) {
  const normalizedPath = import_path4.default.resolve(filePath);
  ...
  try {
    await import_fs3.promises.access(normalizedPath);
  } catch (error) {
    throw new Error(`Import file not found: ${normalizedPath}`);
  }
  const content = await import_fs3.promises.readFile(normalizedPath, "utf-8");
  ...
  switch (ext) {
    ...
    case ".yaml":
    case ".yml":
      data2 = import_js_yaml3.default.load(content);
      break;
    case ".md":
    case ".markdown":
  ...
  if (data2 && typeof data2 === "object") {
    data2 = await resolveImportsInData.call(this, data2, import_path4.default.dirname(normalizedPath));
  }
  if (this.context.importChain) {
...
function parseMarkdownFile(content, filePath) {
  const result = {
    content,
    filePath,
    name: import_path4.default.basename(filePath, import_path4.default.extname(filePath))
  };
  const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
  if (frontmatterMatch) {
    try {
      const frontmatter = import_js_yaml3.default.load(frontmatterMatch[1]);
...
async function resolveImportsInData(data2, baseDir) {
  if (typeof data2 === "string" && data2.startsWith("import:")) {
    const importPath = import_path4.default.resolve(baseDir, data2.substring(7));
  ...
  if (Array.isArray(data2)) {
    ...
    for (const item of data2) {
      if (typeof item === "string" && item.startsWith("import:")) {
        const importPath = import_path4.default.resolve(baseDir, item.substring(7));
        const importedData = await this.loadImportedFile(importPath);
        resolved.push(importedData);
      } else if (item && typeof item === "object" && item.import) {
        const importPath = import_path4.default.resolve(baseDir, item.import);
        const importedData = await this.loadImportedFile(importPath);
        resolved.push(importedData);
      } else {
    ...
    for (const [key, value] of Object.entries(data2)) {
      if (key === "import" && typeof value === "string") {
        const importPath = import_path4.default.resolve(baseDir, value);
        const importedData = await this.loadImportedFile(importPath);
        Object.assign(resolved, importedData);
      } else if (key === "imports" && Array.isArray(value)) {
        for (const imp of value) {
          if (typeof imp === "string") {
            const importPath = import_path4.default.resolve(baseDir, imp);
            const importedData = await this.loadImportedFile(importPath);
            Object.assign(resolved, importedData);
          } else if (imp && typeof imp === "object") {
            const source = imp.import || imp.source;
            if (source) {
              const importPath = import_path4.default.resolve(baseDir, source);
              const importedData = await this.loadImportedFile(importPath);
              if (imp.alias) {
...
async function processImportChain(filePath) {
  ...
  this.context.baseDir = import_path5.default.dirname(filePath);
  ...
  for (const [name, sourceConfig] of Object.entries(this.config.sources)) {
    ...
    for (const pattern of patterns) {
      ...
      if (pattern.match(/^\/.*\/[gimuy]*$/)) {
        ...
        const fullPattern = import_path6.default.isAbsolute(pattern) ? pattern : import_path6.default.join(baseDir, pattern);
        matches = await (0, import_glob.glob)(fullPattern, globOptions);
      }
      files.push(...matches);
    }
    const sourceData = [];
    for (const file of files) {
      const fullPath = import_path6.default.isAbsolute(file) ? file : import_path6.default.resolve(baseDir, file);
      const relativePath = import_path6.default.relative(baseDir, fullPath);
      let parsed;
      try {
        ...
        switch (config2.parser) {
          case "yaml":
            if (typeof parsed === "string") {
              parsed = import_js_yaml4.default.load(parsed);
            }
            break;
          case "json":
            if (typeof parsed === "string") {
        ...
        if (config2.transform) {
          parsed = config2.transform(parsed, {
            path: relativePath,
            name: import_path6.default.basename(relativePath, import_path6.default.extname(relativePath))
          });
        }
        parsed._meta = {
          file: relativePath,
          fullPath,
          source: name,
          lastModified: (await import_fs4.promises.stat(fullPath)).mtime
        };
...
async function processOutputs() {
  ...
  for (const outputConfig of this.config.outputs) {
    ...
    if (outputConfig.template) {
      const templatePath = import_path7.default.resolve(this.config.options?.baseDir || process.cwd(), outputConfig.template);
      const templateContent = await import_fs5.promises.readFile(templatePath, "utf-8");
    ...
    for (const target of targets) {
      const targetPath = import_path7.default.resolve(this.config.options?.baseDir || process.cwd(), target.path);
      await import_fs5.promises.mkdir(import_path7.default.dirname(targetPath), { recursive: true });
      let content;
      ...
      await import_fs5.promises.writeFile(targetPath, finalContent, "utf-8");
      const outputKey = import_path7.default.basename(target.path);
  ...
  async function walkDir2(dir, relativeTo) {
    try {
      ...
      for (const entry of entries) {
        ...
        const relativePath = import_path8.default.relative(relativeTo, fullPath);
        if (entry.isDirectory()) {
...
async function loadConfig(configPath) {
  ...
  let config2 = {};
  if (targetPath) {
    const exists = await import_fs7.promises.access(targetPath).then(() => true).catch(() => false);
    if (!exists) {
      ...
      const ext = import_path9.default.extname(targetPath).toLowerCase();
      const content = await import_fs7.promises.readFile(targetPath, "utf-8");
      switch (ext) {
        case ".js":
        case ".mjs":
          try {
            const module2 = await import(import_path9.default.resolve(targetPath));
            config2 = module2.default || module2;
          } catch (err) {
            try {
              delete require.cache[import_path9.default.resolve(targetPath)];
              const requireResult = require(import_path9.default.resolve(targetPath));
...
${err.message}`);
        ...
        case ".yaml":
        case ".yml":
          config2 = import_js_yaml5.default.load(content);
          break;
        default:
...
function resolveConfig(config2, baseDir) {
  ...
  return {
    ...
    options: {
      ...config2.options,
      baseDir: config2.options?.baseDir ? import_path10.default.isAbsolute(config2.options.baseDir) ? config2.options.baseDir : import_path10.default.resolve(resolveDir, config2.options.baseDir) : resolveDir,
...
function watch(callback) {
  ...
  this.watcher = import_chokidar.default.watch(paths, {
    persistent: true,
    ignoreInitial: true,
    awaitWriteFinish: {
...
var NodeStrategy = class {
  getName() {
    return `Node.js ${process.version} on ${import_os.default.platform()}`;
  }
  getCapabilities() {
    return {
  ...
  getPaths() {
    ...
    return {
      ...
      documents: import_path11.default.join(home, "Documents"),
      downloads: import_path11.default.join(home, "Downloads"),
      desktop: import_path11.default.join(home, "Desktop"),
      // Config directories
      config: this.getConfigDir(),
      cache: this.getCacheDir(),
      temp: import_os.default.tmpdir(),
      ...
      lib: this.getLibDir(),
      // Separators
      sep: import_path11.default.sep,
      delimiter: import_path11.default.delimiter
    };
  }
  normalizePath(filePath) {
    return import_path11.default.normalize(filePath);
  }
  isAbsolutePath(filePath) {
    return import_path11.default.isAbsolute(filePath);
  }
  async createDirectory(dirPath) {
    await import_fs8.promises.mkdir(dirPath, {
      recursive: true,
  ...
  resolveDataDir(appName) {
    ...
    return import_path11.default.join(base, appName);
  }
  ...
  resolveTempDir(appName) {
    return import_path11.default.join(import_os.default.tmpdir(), appName);
  }
  getEnvVars() {
    return { ...process.env };
  }
...
${error.message}`);
  ...
  getUserDataDir() {
    switch (process.platform) {
      case "win32":
        return process.env.APPDATA || import_path11.default.join(import_os.default.homedir(), "AppData", "Roaming");
      case "darwin":
        return import_path11.default.join(import_os.default.homedir(), "Library", "Application Support");
      default:
        return process.env.XDG_DATA_HOME || import_path11.default.join(import_os.default.homedir(), ".local", "share");
    }
  }
  getAppDataDir() {
  ...
  getConfigDir() {
    switch (process.platform) {
      case "win32":
        return process.env.APPDATA || import_path11.default.join(import_os.default.homedir(), "AppData", "Roaming");
      case "darwin":
        return import_path11.default.join(import_os.default.homedir(), "Library", "Preferences");
      default:
        return process.env.XDG_CONFIG_HOME || import_path11.default.join(import_os.default.homedir(), ".config");
    }
  }
  getCacheDir() {
    switch (process.platform) {
      case "win32":
        return process.env.LOCALAPPDATA || import_path11.default.join(import_os.default.homedir(), "AppData", "Local");
      case "darwin":
        return import_path11.default.join(import_os.default.homedir(), "Library", "Caches");
      default:
        return process.env.XDG_CACHE_HOME || import_path11.default.join(import_os.default.homedir(), ".cache");
    }
  }
  getLogsDir() {
    switch (process.platform) {
      case "win32":
        return import_path11.default.join(this.getUserDataDir(), "logs");
      case "darwin":
        return import_path11.default.join(import_os.default.homedir(), "Library", "Logs");
      default:
        return import_path11.default.join(this.getUserDataDir(), "logs");
    }
  }
